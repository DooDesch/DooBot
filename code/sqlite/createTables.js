...
const createReactionMessagesTable = async () => {
  ...
  if (!table["count(*)"]) {
    logger.log("Creating Table: 'reaction_messages'.", "log");
...
const createVoiceStatesTable = async () => {
  ...
  if (!table["count(*)"]) {
    logger.log("Creating Table: 'voice_states'.", "log");
    ...
    sql
      ...
      .run();
    sql
      .prepare(`CREATE UNIQUE INDEX idx_voice_states_id ON voice_states (id)`)
      .run();
    sql
      .prepare(
...
const createRolesTable = async () => {
  ...
  if (!table["count(*)"]) {
    logger.log("Creating Table: 'roles'.", "log");
...
const createSelectMenusTable = async () => {
  ...
  if (!table["count(*)"]) {
    logger.log("Creating Table: 'select_menus'.", "log");
    ...
    sql
      ...
      .run();
    sql
      .prepare(`CREATE UNIQUE INDEX idx_select_menus_id ON select_menus (id)`)
      .run();
    sql
      .prepare(
